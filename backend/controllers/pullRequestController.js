...
export async function listPullRequests(request, response) {
  ...
  if (request.query.queueStatus) {
    if (!QUEUE_STATUSES.includes(request.query.queueStatus)) throw new AppError('Invalid queueStatus filter', 400, 'VALIDATION_ERROR');
    where.queueStatus = request.query.queueStatus;
  }
  if (request.query.ciStatus) {
    if (!CI_STATUSES.includes(request.query.ciStatus)) throw new AppError('Invalid ciStatus filter', 400, 'VALIDATION_ERROR');
  ...
  const queueOrder = literal(`CASE "queue_status" WHEN 'REVIEW_NOW' THEN 1 WHEN 'RETURN_TO_AGENT' THEN 2 WHEN 'WAITING' THEN 3 WHEN 'LOW_RISK' THEN 4 ELSE 5 END`);
  const pullRequests = await PullRequest.findAll({ where, order: request.query.queueStatus ? selectedSort : [[queueOrder, 'ASC'], ...selectedSort] });
...
export async function getPullRequest(request, response) {
  ...
  const repository = pullRequest.Repository || await Repository.findByPk(pullRequest.repositoryId);
