...
export async function createRepository(request, response) {
  ...
  if (!created)
    throw new AppError(
      'This repository is already registered for the team',
      409,
      'REPOSITORY_EXISTS',
    );
...
export async function updateRepository(request, response) {
  ...
  const updates = {};
  if (request.body.criticalPaths !== undefined)
    updates.criticalPaths = normalizeStringArray(request.body.criticalPaths, 'criticalPaths');
  if (request.body.agentAccounts !== undefined)
    updates.agentAccounts = normalizeStringArray(request.body.agentAccounts, 'agentAccounts');
  if (request.body.lowRiskMaxLines !== undefined) {
    ...
    if (!Number.isInteger(limit) || limit < 0 || limit > 10000)
      throw new AppError(
        'lowRiskMaxLines must be an integer from 0 to 10000',
        400,
        'VALIDATION_ERROR',
      );
  ...
  await sequelize.transaction(async (transaction) => {
    ...
    const openPullRequests = await PullRequest.findAll({
...
export async function synchronizeRepository(request, response) {
  ...
  response.json({
    sync: { imported: result.imported, syncedAt: result.syncedAt },
