...
export async function receiveGitHubWebhook(request, response) {
  ...
  if (!secret)
    throw new AppError('GitHub webhooks are not configured', 503, 'GITHUB_WEBHOOK_SECRET_MISSING');
  if (!Buffer.isBuffer(request.body))
    throw new AppError('GitHub webhook payload must be raw JSON', 400, 'WEBHOOK_PAYLOAD_INVALID');
