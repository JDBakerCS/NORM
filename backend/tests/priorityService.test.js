...
import { calculateAge, calculateImpact, calculatePriority, calculateSize, calculateUrgency } from '../services/priorityService.js';
...
test('every size boundary is deterministic', () => {
  const cases = [[0, 0], [1, 2], [50, 2], [51, 5], [200, 5], [201, 10], [500, 10], [501, 15], [1000, 15], [1001, 20]];
  for (const [lines, score] of cases) assert.equal(calculateSize(lines).score, score, `${lines} lines`);
});
...
test('priority never exceeds 100 and reasons match scored components', () => {
  const result = calculatePriority({
    labels: ['priority:critical'], changedLines: 1500, changedFilePaths: ['auth/login.js'], githubCreatedAt: '2026-07-01T00:00:00Z',
  }, { criticalPaths: ['auth/'] }, new Date('2026-08-10T00:00:00Z'));
