...
export async function requireMembership(userId, teamId, roles = null, teamMemberModel = TeamMember) {
  const membership = await teamMemberModel.findOne({ where: { userId, teamId } });
  if (!membership) throw new AppError('You do not have access to this team', 403, 'TEAM_ACCESS_DENIED');
  if (roles && !roles.includes(membership.role)) throw new AppError('Your team role cannot perform this action', 403, 'TEAM_ROLE_REQUIRED');
  return membership;
}
...
export async function requirePullRequestAccess(userId, pullRequestId) {
  const pullRequest = await PullRequest.findByPk(pullRequestId, { include: [{ model: Repository }] });
