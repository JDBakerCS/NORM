...
export function getReviewRequestSummary(pullRequest = {}) {
  ...
  if (pullRequest.reviewStatus === 'APPROVED') return { label: 'Reviewer approved', hasActiveRequest: false };
  if (pullRequest.reviewStatus === 'CHANGES_REQUESTED') return { label: 'Reviewer requested changes', hasActiveRequest: false };
...
export function sortNamedChecks(checkResults = []) {
  ...
  return [...checkResults].sort((left, right) => (order[left.status] ?? 3) - (order[right.status] ?? 3));
}
...
export function getNamedCheckSummary(checkResults = []) {
  ...
  if (failed.length > 0) return { label: `Failed: ${compactNames(failed.map((check) => check.name))}`, status: 'FAILED' };
  if (running.length > 0) return { label: `Running: ${compactNames(running.map((check) => check.name))}`, status: 'RUNNING' };
  if (results.length > 0) return { label: `${results.length} named ${results.length === 1 ? 'check' : 'checks'} passed`, status: 'PASSED' };
