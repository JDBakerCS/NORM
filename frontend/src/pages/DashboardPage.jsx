...
export default function DashboardPage() {
  ...
  useEffect(() => {
    async function loadWorkspace() {
      ...
      } catch (requestError) { setError(errorMessage(requestError, 'Could not load your workspace')); }
  ...
  useEffect(() => {
    ...
    async function loadRepositories() {
      ...
      try {
        ...
        const selected = data.repositories.find((repository) => repository.id === saved) || data.repositories[0];
        setRepositoryId(selected?.id || null);
      } catch (requestError) { setError(errorMessage(requestError, 'Could not load repositories')); }
      finally { setLoading(false); }
    }
    loadRepositories();
  }, [activeTeamId]);
  ...
  useEffect(() => {
    if (!repositoryId) { setPullRequests([]); return; }
  ...
  const visible = useMemo(() => pullRequests.filter((pullRequest) => {
    const matchesSearch = !search || `${pullRequest.title} ${pullRequest.authorLogin}`.toLowerCase().includes(search.toLowerCase());
    const matchesAgent = agentFilter === 'all' || (agentFilter === 'agent' ? pullRequest.isAgentGenerated : !pullRequest.isAgentGenerated);
  ...
  async function sync() {
    ...
    try {
      ...
      const { data } = await api.get(`/teams/${activeTeamId}/repositories`);
      setRepositories(data.repositories);
    } catch (requestError) { setError(errorMessage(requestError, 'Repository could not be synchronized')); }
  ...
  return (
    <div className="dashboard-page">
      <section className="page-heading dashboard-heading">
        <div><p className="eyebrow">Decision queue</p><h1>Pull requests, sorted by next action.</h1><p>Deterministic signals surface what needs a human and explain why.</p></div>
        {repositories.length > 0 && <div className="heading-actions"><RepositorySelector repositories={repositories} selectedId={repositoryId} onChange={setRepositoryId} /><button className="button button-primary" type="button" onClick={sync} disabled={syncing}>{syncing ? <><span className="spinner small" /> Syncing…</> : '↻ Sync from GitHub'}</button></div>}
      </section>
      ...
      {repositories.length === 0 ? (
        ...
        <>
          <section className="queue-summary">
            <div><span>Review-ready</span><strong>{counts.REVIEW_NOW}</strong><small>ranked by priority</small></div>
            <div><span>Needs another pass</span><strong>{counts.RETURN_TO_AGENT}</strong><small>failed, conflicted, or revised</small></div>
            <div><span>Still in motion</span><strong>{counts.WAITING}</strong><small>drafts and running checks</small></div>
            <div><span>Low-risk changes</span><strong>{counts.LOW_RISK}</strong><small>small documentation updates</small></div>
          </section>
